...
type CreatedOrder={id:string;customerEmail:string;product:string;region:string;quantity:number;durationDays:number;amount:number;currency:string};
...
export async function sendOrderCreatedEmails(order:CreatedOrder){
  ...
  const details=[{label:"订单编号",value:order.id,accent:true},{label:"商品 / 服务",value:product},{label:"地区 / 数量",value:`${order.region} × ${order.quantity}`},{label:"服务周期",value:`${order.durationDays} 天`},{label:"订单金额",value:`${order.currency} ${order.amount.toFixed(2)}`}];
