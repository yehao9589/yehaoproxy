type ProxyNoteKey = "CITY" | "ACTIVATED_AT" | "TRANSIT_URL";
...
export function visibleProxyNote(note: string | null | undefined) {
  return String(note || "")
    .replace(/^\[(?:CITY|ACTIVATED_AT|TRANSIT_URL)\][^\n]*$/gm, "")
    .replace(/\n{2,}/g, "\n")
    .trim();
}
...
export function composeProxyNote(
  ...
  const savedTransitUrl = transitUrl === undefined ? proxyNoteValue(existingNote, "TRANSIT_URL") : transitUrl.trim();
  return [
    cleanVisible,
    savedCity ? `[CITY]${savedCity}` : "",
    activatedAt ? `[ACTIVATED_AT]${activatedAt}` : "",
    savedTransitUrl ? `[TRANSIT_URL]${savedTransitUrl}` : "",
  ].filter(Boolean).join("\n") || null;
}
