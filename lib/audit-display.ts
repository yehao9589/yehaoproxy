...
const actions: Record<string, string> = {
  ...
  "customer.create":"手动创建客户","customer.update":"修改客户资料","customer.password.update":"修改客户密码","customer.credit.update":"调整客户信用额度","customer.impersonate":"管理员登录客户面板",
  "wallet.adjust":"客户余额调账","order.confirm":"确认订单收款","order.cancel":"取消订单","order.customer_cancel":"客户取消订单","order.fulfill":"发放代理资源","order.refund":"订单退款","order.update":"修改订单信息","order.service_update":"修改订单服务配置","order.amount.update":"修改待付款订单金额","order.wallet_credit_pay":"使用余额或信用额度支付订单","order.auto_extract":"自动交付代理资源","order.manual_create":"手动创建账单",
  "payment.alipay.success":"支付宝付款成功","payment.alipay.failed":"支付宝回调处理失败",
  "proxy.update":"修改代理资料","proxy.resource.update":"修改代理资源信息","proxy.resource.delete":"删除已分配资源","proxy.bulk_credentials":"批量修改代理账号密码","proxy.renewal_orders.create":"创建代理续费订单","proxy.bulk_renew_complete":"批量续费完成","proxy.export":"导出代理资源",
  "inventory.import":"导入代理库存","inventory.update":"修改库存资源","ticket.create":"创建客户工单","ticket.reply":"回复客户工单","ticket.update":"修改工单","ticket.customer_close":"客户关闭工单",
  ...
  "coupon.create":"创建优惠券","coupon.update":"修改优惠券","backup.create":"创建系统备份","backup.import":"导入系统备份","backup.delete":"删除系统备份","update.check":"检查系统更新","update.trigger":"执行系统更新","update.rollback":"回滚系统版本","update.settings.save":"保存更新设置",
  "whitelist.create":"添加 IP 白名单","whitelist.delete":"删除 IP 白名单",
};

const resources: Record<string, string> = {
  auth:"账户认证",admin:"管理员账户",customer:"客户",wallet:"账户余额",order:"订单",proxy:"代理资源",inventory:"库存资源",service_request:"售后申请",ticket:"工单",notification:"通知",email:"邮件",scheduled_task:"定时任务",database:"数据库",node:"节点服务",site_config:"站务配置",coupon:"优惠券",whitelist:"IP 白名单",system_backup:"系统备份",system_update:"系统更新",
};
...
const keys: Record<string, string> = {
  ...
  sourceOrderId:"原服务订单",targetOrderId:"目标订单",orderId:"订单号",orderIds:"订单列表",bundleOrderId:"合并账单号",txId:"交易流水号",tradeNo:"支付平台流水号",destination:"退款去向",
  ...
  payable:"支付金额",discount:"优惠金额",balanceAfter:"操作后余额",creditUsed:"使用信用额度",creditLimit:"信用额度",total:"合计金额",product:"商品",productType:"商品类型",region:"地区",quantity:"数量",count:"数量",
  ...
  currentVersion:"当前版本",currentCommit:"当前版本提交",remoteCommit:"最新版本提交",channel:"更新通道",image:"镜像地址",fileName:"备份文件",type:"类型",value:"数值",code:"优惠码",enabled:"是否启用",storage:"存储位置",size:"文件大小",permissions:"权限范围",sessionsRevoked:"其他登录会话",previousStatus:"原状态",priority:"优先级",assignedAdminId:"指派管理员",relatedService:"关联服务",internal:"内部回复",usernameChanged:"是否修改账号",passwordChanged:"是否修改密码",issues:"核验问题",manual:"处理方式",linkedBillId:"关联账单",billingMode:"计费方式",refund:"退款结果",serverId:"VPS 服务器",ip:"IP 地址",buckets:"各提醒阶段",
};
...
export function auditDetailText(raw:string|null|undefined,resourceType="") {
  ...
  try{
    ...
    if(resourceType==="email")return`${displayValue("to",data.to)} · ${displayValue("subject",data.subject)}`;
