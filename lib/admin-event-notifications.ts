...
const DEFAULT_ADMIN_TEMPLATES=[
  ...
  {id:"admin_renewal",enabled:true,emailEnabled:true,emailSubject:"收到续费订单 {{orderId}}",emailBody:"客户 {{customerEmail}} 提交了续费订单，原服务 {{sourceOrderId}}，续费周期 {{durationDays}} 天，金额 {{amount}}。"},
...
export async function notifyAdmins(event:AdminEvent,values:Values,details:Array<{label:string;value:string;accent?:boolean}>=[]){
  ...
  const subject=render(template.emailSubject,values),body=render(template.emailBody,values),origin=String(process.env.PUBLIC_APP_URL||"").replace(/\/$/,"");
