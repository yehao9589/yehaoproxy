...
也可以直接在宝塔新建编排并粘贴下面的完整示例：

```yaml
services:
  yehaoproxy:
    # GitHub Container Registry 正式版镜像；生产环境建议固定版本号。
    image: ${YEHAOPROXY_IMAGE:-ghcr.io/yehao9589/yehaoproxy:v1.0.0}
    container_name: yehaoproxy

    # 单容器启动器会统一管理 Web、数据库桥接、X-Panel 桥接和定时任务。
    command: ["node", "scripts/single-container.mjs"]

    # 使用宿主机网络，容器才能通过 127.0.0.1 访问宝塔 MySQL。
    network_mode: host
...
```dotenv
# 正式版本镜像；升级时只需要修改这里的版本号。
YEHAOPROXY_IMAGE=ghcr.io/yehao9589/yehaoproxy:v1.0.0

# 没有域名时可先使用 http://服务器IP:3000；配置证书后改为 HTTPS 域名。
