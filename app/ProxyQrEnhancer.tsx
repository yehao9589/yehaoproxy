  ...
  return <div className="proxy-usage-mask" onMouseDown={e=>{if(e.target===e.currentTarget){setItem(null);setError("")}}}>
    <section className="proxy-usage-modal">
      {error?<><header><h2>加载失败</h2><button onClick={()=>setError("")}>×</button></header><p className="live-error">{error}</p></>:item&&<>
        <header><div><small>STATIC IP CONNECTION</small><h2>静态 IP 使用</h2><p>{item.region} · {item.protocol.toUpperCase()} · {item.host}:{item.port}</p></div><button onClick={()=>setItem(null)}>×</button></header>
        <div className="usage-body">
          ...
          <aside className="usage-qr"><span>SHADOWROCKET</span><h3>扫码快速使用</h3><dl><div><dt>节点名称</dt><dd>{nodeName}</dd></div><div><dt>节点协议</dt><dd>{item.protocol.toUpperCase()}</dd></div></dl>{qr&&<img src={qr} alt={`${nodeName} 代理二维码`}/>}<code className="usage-qr-uri">{qrUri}</code><p>二维码包含当前代理地址、账号密码和节点名称，打开小火箭点击右上角扫码即可添加。</p></aside>
        </div>
      </>}
    </section>
  </div>;
