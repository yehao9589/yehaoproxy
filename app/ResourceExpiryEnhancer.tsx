...
export default function ResourceExpiryEnhancer(){
  useEffect(()=>{
    ...
    function enhance(){
      ...
      if(!grid||form.querySelector('input[name="expiresAt"]'))return;
      ...
      fetch(`/api/admin/orders/${encodeURIComponent(orderId)}/resources/by-address?host=${encodeURIComponent(host)}&port=${encodeURIComponent(port)}`).then(async response=>({ok:response.ok,data:await response.json()})).then(({ok,data})=>{if(ok&&data.expiresAt)input.value=localMinute(data.expiresAt)}).catch(()=>{});
