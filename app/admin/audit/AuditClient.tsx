...
const names: Record<string,string> = {
  ...
  "admin.create":"创建管理员","customer.create":"手动创建客户","customer.update":"修改客户资料","customer.credit.update":"调整客户信用额度",
  ...
  "order.service_update":"修改订单服务配置","order.amount.update":"修改待付款订单金额","proxy.update":"修改代理资料","inventory.import":"导入代理库存",
  "site.config.update":"修改站务配置","site.logo.upload":"上传站点 Logo","site.logo.delete":"删除站点 Logo",
...
const detailKeys: Record<string,string> = {
  ...
  beforeAmount:"修改前金额",afterAmount:"修改后金额",currency:"订单币种",type:"文件类型",size:"文件大小",storage:"存储位置",fields:"修改字段",
};
const fieldNames:Record<string,string>={siteName:"站点名称",logoText:"Logo 文字",logoUrl:"Logo 图片",topAdEnabled:"顶部广告开关",topAdText:"顶部广告文字",topAdLink:"顶部广告链接",footerDescription:"底部说明",companyName:"公司名称",supportEmail:"客服邮箱",copyright:"版权信息",icpNumber:"备案号"};
const currencies:Record<string,string>={CNY:"人民币（CNY）",USD:"美元（USD）",EUR:"欧元（EUR）",GBP:"英镑（GBP）",JPY:"日元（JPY）"};
...
function detailValue(key:string,value:unknown){if(value===null||value===undefined||value==="")return"无";if(key==="currency")return currencies[String(value).toUpperCase()]||String(value);if(key==="size"&&Number.isFinite(Number(value)))return`${(Number(value)/1024).toFixed(1)} KB`;if(key==="storage")return value==="filesystem"?"服务器文件目录":value==="database"?"数据库备用存储":String(value);if(key==="type"){const types:Record<string,string>={"image/png":"PNG 图片","image/jpeg":"JPG 图片","image/webp":"WEBP 图片"};return types[String(value)]||String(value)}if(key==="fields"&&Array.isArray(value))return value.length?value.map(item=>fieldNames[String(item)]||String(item)).join("、"):"无";if(key==="beforeAmount"||key==="afterAmount"||key==="amount")return Number.isFinite(Number(value))?Number(value).toFixed(2):String(value);if(typeof value==="boolean")return value?"是":"否";if(Array.isArray(value))return value.length?value.map(item=>String(item)).join("、"):"无";if(typeof value==="object")return"配置已更新";if(typeof value==="string"&&/^[0-9a-f]{8}-[0-9a-f-]{27,}$/i.test(value))return"内部记录";if(typeof value==="string"&&/^\d{4}-\d{2}-\d{2}T/.test(value)){const date=new Date(value);if(!Number.isNaN(date.getTime()))return date.toLocaleString("zh-CN",{hour12:false})}return String(value)}
function detail(log:Log){if(!log.detail)return"—";try{const value=JSON.parse(log.detail);if(log.resourceType==="email")return`${value.to||"未知收件人"} · ${value.subject||""}`;if(log.resourceType==="scheduled_task")return`扫描 ${value.scanned??0}，站内通知 ${value.created??0}，邮件 ${value.emailed??0}`;const hidden=new Set(["allocationId","allocationIds","resourceId","resourceIds","passwordHash","encryptedPassword","token","tokenHash","credentialRef"]);return Object.entries(value).filter(([key])=>!hidden.has(key)).slice(0,5).map(([key,item])=>`${detailKeys[key]||key}：${detailValue(key,item)}`).join("；")||"操作已记录"}catch{return"操作已记录"}}
function resourceName(log:Log){const base=resources[log.resourceType]||"系统资源";if(log.resourceType==="customer"&&log.resourceCustomerName)return`客户 ${log.resourceCustomerName}`;if(log.resourceType==="site_config")return"站务配置";if(log.resourceType!=="proxy")return`${base}${log.resourceId?` ${customerNo(log.resourceId)}`:""}`;if(log.resourceDisplay)return`代理资源 ${log.resourceDisplay}`;let value:any={};try{value=JSON.parse(log.detail||"{}")||{}}catch{}const address=value.address||value.previousAddress||(value.host&&value.port?`${value.host}:${value.port}`:value.host);if(address)return`代理资源 ${address}`;return log.resourceId?`代理资源 #${String(log.resourceId).slice(0,8)}`:"代理资源"}
