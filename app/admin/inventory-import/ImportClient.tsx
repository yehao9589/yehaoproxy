...
export default function ImportClient(){
  ...
  return <div className="inventory-workbench">
    ...
    <div className="import-layout"><form className="import-main" onSubmit={submit}>
      ...
      <div className="import-section"><div className="import-section-title"><span>2</span><div><h3>设置库存属性</h3><p>以下属性将应用于本次导入的全部代理</p></div></div><div className="import-fields"><label>产品类型<select name="product"><option value="static-isp">静态住宅 ISP</option><option value="residential">动态住宅</option><option value="datacenter">数据中心</option><option value="mobile">移动代理</option></select></label><label>国家 / 地区<input name="country" defaultValue="US" maxLength={2} required/><small>ISO 两位国家代码</small></label><label>城市<input name="city" placeholder="可选，例如 New York"/></label><label>协议<select name="protocol"><option>HTTPS</option><option>HTTP</option><option>SOCKS5</option></select></label><label>采购成本（可选）<input name="cost" type="number" min="0" step="0.01" placeholder="不填写则不记录"/></label></div></div>
      ...
      <div className="import-actions"><span>提交前请确认国家、产品类型和连接信息正确</span><button className="primary" disabled={submitting||!valid.length||!!invalid.length||!!localDuplicates}>{submitting?"正在安全入库…":`确认入库 ${valid.length} 条`}</button></div>
    </form>
