...
export default function ServicePriceSettings(){
  ...
  useEffect(()=>{fetch("/api/admin/settings").then(r=>r.json()).then(d=>{
    ...
    if(d.options?.ipReplacementFreeDays!=null)setFreeDays(d.options.ipReplacementFreeDays);
    if(d.options?.ipReplacementFreeCount!=null)setFreeCount(d.options.ipReplacementFreeCount);
    setCredentialEditing(d.options?.customer_node_credential_editing==="true");
  }).catch(()=>setMessage("配置加载失败，请刷新后重试"))},[]);
  async function save(){
    setSaving(true);setMessage("");
    const settings=[["nodeTrafficResetPrice",resetPrice],["ipReplacementPrice",replacePrice],["ipReplacementFreeDays",freeDays],["ipReplacementFreeCount",freeCount],["customer_node_credential_editing",String(credentialEditing)]];
    ...
    <section className="setting-card service-policy-card">
      ...
      <div className="policy-section">
        ...
        <div className="policy-price-grid">
          ...
          <label><span>超过免费期更换 IP<small>免费期或免费次数用完后的单次费用</small></span><div><em>¥</em><input aria-label="付费更换 IP 价格" type="number" min="0" step="0.01" value={replacePrice} onChange={e=>setReplacePrice(e.target.value)}/><b>元 / 次</b></div></label>
          <label><span>免费更换有效期<small>从 IP 提取时间开始计算</small></span><div><input aria-label="免费更换有效天数" type="number" min="0" max="365" step="1" value={freeDays} onChange={e=>setFreeDays(e.target.value)}/><b>天内</b></div></label>
          <label><span>免费更换次数<small>每条已提取 IP 在有效期内可用次数</small></span><div><input aria-label="免费更换次数" type="number" min="0" max="100" step="1" value={freeCount} onChange={e=>setFreeCount(e.target.value)}/><b>次</b></div></label>
        </div>
        <div className="policy-tip"><b>免费规则</b><span>IP 提取后 {freeDays||0} 天内可免费更换 {freeCount||0} 次；次数用完或超过有效期后，按上方价格收费。</span></div>
      </div>
      <div className="policy-section">
