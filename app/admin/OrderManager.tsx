...
export default function OrderManager({ search = "", kind = "all" }: { search?: string; kind?: "all" | "bills" | "products" }) {
  ...
  const workflowGroup=(order:Order)=>order.status==="pending"?"unpaid":order.status==="refunded"?"refunded":["failed","cancelled","canceled"].includes(order.status)?"cancelled":billKind(order)==="renewal"&&!order.adminNote?.includes("[RENEWAL_VERIFIED_AT]")?"verification":billKind(order)==="after-sales"&&order.status!=="active"?"processing":billKind(order)==="purchase"&&["paid","provisioning"].includes(order.status)?"delivery":"completed";
