...
type AllocatedService={id:string;orderId:string;kind:string;address:string;wifiName?:string|null;country?:string|null;city?:string|null};
...
export default function OrderManager({ search = "", kind = "all" }: { search?: string; kind?: "all" | "bills" | "products" }) {
  ...
  async function load() {
    ...
    try {
      ...
      const [response, serviceResponse,requestResponse] = await Promise.all([fetch("/api/admin/orders?size=100",requestOptions), fetch("/api/admin/services",requestOptions),fetch("/api/admin/service-requests",requestOptions)]);
      ...
      if (!serviceResponse.ok) throw new Error(services.error || "服务资源读取失败");
      if (!requestResponse.ok) throw new Error(requests.error || "售后状态读取失败");
      const servicesByOrder = new Map<string,AllocatedService[]>();
      for(const service of (services.items||[]) as AllocatedService[]){if(service.kind!=="proxy")continue;const group=servicesByOrder.get(service.orderId)||[];group.push(service);servicesByOrder.set(service.orderId,group)}
      ...
      setRows((data.items || []).map((item: Order) => { const allocatedServices=servicesByOrder.get(item.id)||[],service=allocatedServices[0]; return { ...item, allocatedServices, serviceRequestStatus:requestByBill.get(item.id)||null,allocatedIp: service?.address || null, allocatedWifiName: service?.wifiName || null, allocatedCountry: service?.country || null, allocatedCity: service?.city || null }; }));
    } catch (reason) { setError(reason instanceof Error ? reason.message : "订单读取失败"); return false; }
  }
  useEffect(() => { void load(); }, []);

  async function open(id: string) {
    setBusy(true); setError("");
    try { const response = await fetch(`/api/admin/orders/${encodeURIComponent(id)}`,{cache:"no-store"}); const data = await response.json(); if (!response.ok) throw new Error(data.error || "订单详情读取失败"); setDetail(data); }
  ...
  const billStats = {
    unpaid: scoped.filter((item) => workflowGroup(item) === "unpaid").length,
    attention: scoped.filter((item) => ["delivery", "verification", "processing"].includes(workflowGroup(item))).length,
    completed: scoped.filter((item) => workflowGroup(item) === "completed").length,
    received: scoped.filter((item) => !["pending", "failed"].includes(item.status)).reduce((sum, item) => sum + Number(item.paidAmount ?? item.amount), 0),
  };
  const toolbar = <div className="module-toolbar"><div>{kind === "all" && <><button className={effectiveKind === "bills" ? "on" : ""} onClick={() => { setCategory("bills"); setStatusFilter("all"); }}>账单管理</button><button className={effectiveKind === "products" ? "on" : ""} onClick={() => { setCategory("products"); setStatusFilter("all"); }}>产品订单</button></>}{effectiveKind === "bills" && statusTabs.map(([value, label, count]) => <button key={value} className={statusFilter === value ? "on" : ""} onClick={() => setStatusFilter(value)}>{label} {count}</button>)}</div><AdminRefreshButton/></div>;

  if (effectiveKind === "products") return <div className="product-order-center business-page"><section className="product-order-hero business-hero"><div><small>PRODUCT OPERATIONS</small><h2>产品订单</h2><p>每一行代表一项实际购买的产品服务，集中完成交付与服务信息核对。</p></div><AdminRefreshButton/></section><section className="product-order-list business-workbench"><header><div><h3>产品服务明细</h3><p>可按订单号、结算账单、客户、商品或地区搜索。</p></div><div><input value={search || externalSearch} onChange={(event) => setExternalSearch(event.target.value)} placeholder="搜索产品订单"/><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">全部状态</option>{Object.entries(statusNames).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></header><div className="product-order-table"><div className="product-order-row head"><span>序号</span><span>服务单号</span><span>客户</span><span>代理地址</span><span>WiFi 名称</span><span>国家 / 地区</span><span>产品金额</span><span className="time-sort-head">下单时间 {sortButton}</span><span>状态</span><span>操作</span></div>{visible.map((order,index) => <div className="product-order-row" key={order.id}><span className="table-sequence">{(currentPage-1)*pageSize+index+1}</span><span><button className="order-number-link" onClick={() => void open(order.id)}>{order.id}</button><small>{order.billingOrderId ? `结算账单 ${order.billingOrderId}` : "独立购买"}</small></span><span><b>{order.customerEmail}</b></span><span><b className="mono">{(order.allocatedServices?.length||0)>1?`已分配 ${order.allocatedServices!.length} 个 IP`:order.allocatedIp||"尚未分配"}</b><small>{productName(order.product)} · 购买 {order.quantity} 个</small></span><span><b>{(order.allocatedServices?.length||0)>1?"见资源明细":order.allocatedWifiName||"未设置"}</b></span><span><b>{(order.allocatedServices?.length||0)>1?"见资源明细":regionName(order.allocatedCountry||order.region)}</b><small>{(order.allocatedServices?.length||0)>1?"各 IP 地区单独展示":order.allocatedCity||order.allocatedCountry||order.region}</small></span><span className="product-order-money">¥{Number(order.displayAmount ?? order.amount).toFixed(2)}</span><span>{new Date(order.createdAt).toLocaleString("zh-CN", { hour12: false })}</span><span><b className={`order-status ${order.status}`}>{statusNames[order.status] || order.status}</b></span><span className="product-order-actions"><button onClick={() => void open(order.id)}>{["paid", "provisioning"].includes(order.status) ? "开通服务" : "查看详情"}</button></span>{(order.allocatedServices?.length||0)>1&&<details className="product-resource-details"><summary>展开 {order.allocatedServices!.length} 个 IP 资源</summary><div className="product-resource-grid resource-head"><span>序号</span><span>代理地址</span><span>WiFi 名称</span><span>国家 / 城市</span></div>{order.allocatedServices!.map((service,index)=><div className="product-resource-grid" key={service.id||service.address}><span>{index+1}</span><span>{service.address}</span><span>{service.wifiName||"未设置"}</span><span>{[regionName(service.country||order.region),service.city].filter(Boolean).join(" / ")}</span></div>)}</details>}</div>)}{!visible.length && <div className="empty">暂无匹配的产品订单</div>}</div></section>{busy && <div className="customer-drawer-mask"><div className="customer-drawer loading">正在加载产品订单…</div></div>}{detailView}</div>;

  return <div className="order-manager bill-ledger business-page"><section className="business-hero"><div><small>BILLING OPERATIONS</small><h2>账单管理</h2><p>统一查看收款状态与业务进度；产品交付、续费核验和售后处理分别进入对应业务页面。</p></div><AdminRefreshButton/></section><section className="business-metrics"><article><i>单</i><span><small>全部账单</small><b>{scoped.length}</b><em>累计业务账单</em></span></article><article><i className="warning">付</i><span><small>等待付款</small><b>{billStats.unpaid}</b><em>等待客户完成支付</em></span></article><article><i className="attention">办</i><span><small>需要跟进</small><b>{billStats.attention}</b><em>交付、核验或售后处理中</em></span></article><article><i className="success">收</i><span><small>确认收款</small><b>¥{billStats.received.toFixed(2)}</b><em>{billStats.completed} 张账单已完成</em></span></article></section><section className="module business-workbench bill-workbench">{toolbar}{error && <div className="live-error">{error}<button onClick={() => setError("")}>×</button></div>}<div className="bill-logic-note"><b>账单只负责收款与查看进度</b><span>新购服务请在“产品订单”交付；续费只在“续费订单”核验；一次性服务由“售后申请”处理。</span></div><div className="admin-table"><div className="arow order ahead"><span>序号</span><span>账单号</span><span>客户</span><span>账单内容</span><span>类型</span><span>金额 / 付款</span><span className="time-sort-head">下单时间 {sortButton}</span><span>收款状态</span><span>业务进度</span><span>操作</span></div>{visible.map((order,index) => {const summary=billSummary(order),kind=billKind(order);return <div className="arow order" key={order.id}><span className="table-sequence">{(currentPage-1)*pageSize+index+1}</span><span><button className="order-number-link" onClick={() => void open(order.id)}>{order.id}</button><small>{order.paymentReference||"暂无支付流水"}</small></span><span><b>{order.customerName||"未设置名称"}</b><small>{order.customerEmail}</small></span><span><b>{summary.products}</b><small>{summary.regions}</small></span><span><em className={`bill-kind ${kind}`}>{billKindNames[kind]}</em></span><span><b>¥{order.amount.toFixed(2)}</b><small>{order.paymentMethod?methods[order.paymentMethod]||order.paymentMethod:"未选择付款方式"}</small></span><span>{new Date(order.createdAt).toLocaleString("zh-CN", { hour12: false })}</span><span><b className={`bill-financial ${workflowGroup(order)}`}>{financialStatus(order)}</b></span><span><b className={`bill-progress ${workflowGroup(order)}`}>{businessProgress(order)}</b></span><span className="live-actions"><button onClick={() => void open(order.id)}>{billActionName(order)}</button></span></div>})}{!visible.length && <div className="empty">{rows.length ? "没有找到匹配的账单" : "暂无账单"}</div>}</div>{busy && <div className="customer-drawer-mask"><div className="customer-drawer loading">正在加载账单详情…</div></div>}{detailView}</section></div>;
}
