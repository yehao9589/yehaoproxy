...
export async function GET(req:Request){
  ...
  const owned=await db.select({allocation:proxyAllocations,product:orders.product,region:orders.region,countryName:productOffers.regionName,city:inventory.city,durationDays:orders.durationDays}).from(proxyAllocations).innerJoin(orders,eq(proxyAllocations.orderId,orders.id)).leftJoin(productOffers,and(eq(productOffers.product,orders.product),eq(productOffers.region,orders.region))).leftJoin(inventory,and(eq(inventory.host,proxyAllocations.host),eq(inventory.port,proxyAllocations.port),eq(inventory.reservedByOrderId,orders.id))).where(and(eq(orders.customerEmail,user.email),eq(proxyAllocations.status,"active"),ne(orders.status,"refunded"))).orderBy(desc(proxyAllocations.expiresAt)).limit(1000),items=[];
