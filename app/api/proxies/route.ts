...
export async function GET(req:Request){
  ...
  for(const row of owned)items.push({...row.allocation,password:reveal?await decryptCredential(row.allocation.encryptedPassword):row.allocation.encryptedPassword?"••••••••":null,encryptedPassword:undefined,product:row.product,region:row.region,countryName:row.countryName||row.region,city:row.city||null,durationDays:row.durationDays});
