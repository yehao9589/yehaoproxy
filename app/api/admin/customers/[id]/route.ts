...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const nodeAssets=orderRows.filter(row=>["computer-node","soft-router"].includes(row.product)&&serviceStatuses.has(row.status)&&!row.adminNote?.includes("[RENEWAL_OF]")).map(row=>({
    id:`node:${row.id}`,kind:"node" as const,orderId:row.id,host:null,port:null,username:null,wifiName:null,protocol:"订阅服务",status:row.status,expiresAt:row.expiresAt,autoRenew:row.autoRenew,product:row.product,region:row.region,country:row.region,city:null,
