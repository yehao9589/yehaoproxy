...
export async function GET(){
 ...
 const proxies=proxyRows.map(row=>({kind:"proxy",...row,orderAdminNote:undefined,billingCycle:billingCycleFromNote(row.orderAdminNote),note:undefined,country:row.region,city:row.note?.match(/\[CITY\]([^\n]*)/)?.[1]||null,address:`${row.host}:${row.port}`}));
 const nodes=nodeRows.filter(row=>["computer-node","soft-router"].includes(row.product)&&!row.adminNote?.includes("[RENEWAL_OF]")).map(row=>({kind:"node",id:row.orderId,...row,billingCycle:billingCycleFromNote(row.adminNote),adminNote:undefined,address:null,protocol:null,country:null,city:null}));
