...
export async function GET(){
 ...
 const nodes=nodeRows.filter(row=>["computer-node","soft-router"].includes(row.product)&&!databaseText(row.adminNote).includes("[RENEWAL_OF]")).map(row=>({kind:"node",id:row.orderId,...row,billingCycle:billingCycleFromNote(row.adminNote),adminNote:undefined,address:null,wifiName:(bindingState.orders[row.orderId]||[]).map(target=>target.name).filter(Boolean).join("、")||null,protocol:null,country:null,city:null}));
