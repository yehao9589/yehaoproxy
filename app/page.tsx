...
export default function Home() {
  ...
  const total = useMemo(() => regions[selected].price * quantity * multiplier, [selected, quantity, multiplier]);
  ...
  const buy = `/buy?product=${product}&region=${regions[selected].code}&duration=${duration}&quantity=${quantity}`;
  ...
  return <main>
    ...
    <section id="pricing" className="section pricing">
      ...
      <div className="benefits"><span>✓ 独享使用</span><span>✓ 地区可选</span><span>✓ 多周期支持</span><span>✓ 售后服务</span><span>✓ 7×24 工单支持</span></div>
      <div className="shop"><div className="catalog"><div className="selected-product-banner"><span>{isNode ? "节点服务" : "代理 IP"}</span><b>{currentProduct.name}</b><button onClick={() => document.getElementById("products")?.scrollIntoView()}>更换产品</button></div><div className="tabs">{[7,30,90].map(day => <button key={day} className={duration === day ? "on" : ""} onClick={() => setDuration(day)}>{day} 天</button>)}</div><div className="region-head"><b>国家 / 地区</b><span>参考单价</span></div><div className="regions">{regions.map((region,index) => <button key={region.code} className={selected === index ? "selected" : ""} onClick={() => setSelected(index)}><span className="flag">{region.flag}</span><span><b>{region.country}</b><small>{region.city}</small></span><i className="green">可选</i><strong>${(region.price*multiplier).toFixed(2)}<small>/ {isNode ? "节点" : "IP"}</small></strong></button>)}</div></div><aside className="checkout"><span className="kicker">订单配置</span><h3>{regions[selected].flag} {regions[selected].country} · {currentProduct.name}</h3><dl><div><dt>商品类目</dt><dd>{isNode ? "节点服务" : "代理 IP"}</dd></div><div><dt>有效期</dt><dd>{duration} 天</dd></div><div><dt>交付方式</dt><dd>{isNode ? "后台人工开通" : "额度提取"}</dd></div></dl><label>购买数量</label><div className="stepper"><button onClick={() => setQuantity(Math.max(1,quantity-1))}>−</button><input value={quantity} onChange={event => setQuantity(Math.min(500,Math.max(1,Number(event.target.value)||1)))}/><button onClick={() => setQuantity(Math.min(500,quantity+1))}>＋</button></div><div className="total"><span>参考金额<small>结算时按商品配置计算</small></span><b>${total.toFixed(2)}</b></div><a className="primary buy" href={buy}>创建订单 →</a><p className="secure">余额支付 · 后台可人工确认 · 售后可追踪</p></aside></div>
    </section>
