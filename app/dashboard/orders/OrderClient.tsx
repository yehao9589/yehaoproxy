...
const statusLabel=(order:O)=>order.product==="node-traffic-reset"?(["paid","provisioning"].includes(order.status)?"等待重置流量中":order.status==="active"?"流量重置完成":labels[order.status]||order.status):nodeProducts.has(order.product)&&["paid","provisioning"].includes(order.status)?"待开通":labels[order.status]||order.status;
  ...
  return <div className="customer-page order-customer-page">
    ...
    <div className="standalone-table order-customer-table">
      ...
      {items.length===0?<div className="empty">暂无订单</div>:items.map(o=><div className="orow" key={o.id}>
        <span className="mono"><button className="customer-order-number" onClick={()=>setDetail(o)}>{o.id}</button></span><span>{productNames[o.product]||o.product}</span><span><b>{o.region} × {o.quantity}</b><small>{o.durationDays} 天{nodeProducts.has(o.product)?"":"，提取后计时"}</small></span><span>${o.amount.toFixed(2)}</span><span>{new Date(o.createdAt).toLocaleString()}</span><span><em className={`customer-order-status ${o.status}`}>{statusLabel(o)}</em></span>
