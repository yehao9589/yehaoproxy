...
export default function OrderClient(){
  ...
  async function payAlipay(order:O){
    setPaying(order.id);setMessage("");
    const response=await fetch("/api/checkout/alipay",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({orderId:order.id})}),data=await response.json();
    setPaying(null);
    if(!response.ok)return setMessage(data.error||"创建支付宝支付失败");
    if(!data.redirectUrl)return setMessage("支付宝未返回支付地址");
