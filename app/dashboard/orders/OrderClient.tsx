...
export default function OrderClient(){
  ...
  const[requestDetail,setRequestDetail]=useState<ServiceRequest|null>(null);
  ...
  function openCheckout(order:O){setCheckout(order);setExternalPaymentOrderId(null);setCouponCode("");setCouponMessage("");setDiscount(0)}
  ...
  return <div className="customer-page order-customer-page">
    ...
    {detail&&<div className="customer-payment-mask customer-order-detail-mask" onMouseDown={event=>{if(event.target===event.currentTarget)setDetail(null)}}><section className="customer-order-detail"><header><div><small>订单详情</small><h2>订单详情</h2><p>{detail.id}</p></div><button type="button" aria-label="关闭订单详情" title="关闭" onClick={()=>setDetail(null)}>×</button></header><dl><div><dt>产品</dt><dd>{productLabel(detail)}</dd></div>{detail.renewalOf&&<div><dt>续费原服务</dt><dd>{detail.renewalOf}</dd></div>}{groupedBundleItems(detail.bundleItems).map(item=><div key={`${item.product}-${item.region}-${item.durationDays}`}><dt>{productNames[item.product]||item.product}</dt><dd>{regionLabel(item.region)} × {item.quantity} · {periodLabel(item.durationDays,item.billingCycle||detail.billingCycle||"fixed-days")} · ${item.amount.toFixed(2)}</dd></div>)}{!detail.bundleItems&&<><div><dt>{nodeProducts.has(detail.product)?"服务范围":"地区 / 数量"}</dt><dd>{regionLabel(detail.region)} × {detail.quantity}</dd></div><div><dt>服务周期</dt><dd>{periodLabel(detail.durationDays,detail.billingCycle||"fixed-days")}{nodeProducts.has(detail.product)?"":"（开通后计时）"}</dd></div></>}{detail.expiresAt&&<div><dt>到期时间</dt><dd>{new Date(detail.expiresAt).toLocaleString("zh-CN",{hour12:false})}</dd></div>}<div><dt>订单金额</dt><dd>${detail.amount.toFixed(2)}</dd></div><div><dt>创建时间</dt><dd>{new Date(detail.createdAt).toLocaleString()}</dd></div><div><dt>订单状态</dt><dd><em className={`customer-order-status ${detail.status}`}>{statusLabel(detail)}</em></dd></div></dl><footer>{detail.status==="pending"&&<button className="danger" disabled={closing===detail.id} onClick={()=>void closeOrder(detail)}>{closing===detail.id?"正在关闭…":"关闭订单"}</button>}{detail.status==="pending"&&<button className="primary" onClick={()=>{setDetail(null);openCheckout(detail)}}>立即支付</button>}{["paid","provisioning","active"].includes(detail.status)&&<a className="primary" href="/dashboard?tab=proxies">{nodeProducts.has(detail.product)?"查看节点服务":"查看代理资源"}</a>}</footer></section></div>}
    {requestDetail&&<div className="customer-payment-mask customer-order-detail-mask" onMouseDown={event=>{if(event.target===event.currentTarget)setRequestDetail(null)}}><section className="customer-order-detail customer-request-detail"><header><div><small>售后申请详情</small><h2>{requestDetail.type==="replace"?"更换 IP":requestDetail.type==="reset_traffic"?"流量重置":requestDetail.type==="renew"?"服务续费":"一次性服务"}</h2><p>{requestDetail.id}</p></div><button type="button" aria-label="关闭售后详情" title="关闭" onClick={()=>setRequestDetail(null)}>×</button></header><dl><div><dt>申请状态</dt><dd><em className={`service-request-status ${requestDetail.status}`}>{requestDetail.status==="pending"?"待处理":requestDetail.status==="approved"?"已批准":requestDetail.status==="completed"?"已完成":requestDetail.status==="rejected"?"已拒绝":"已取消"}</em></dd></div><div><dt>提交时间</dt><dd>{new Date(requestDetail.createdAt).toLocaleString("zh-CN",{hour12:false})}</dd></div><div><dt>申请费用</dt><dd>¥{Number(requestDetail.amount||0).toFixed(2)}</dd></div><div><dt>申请原因</dt><dd>{requestDetail.reason||"未填写"}</dd></div>{requestDetail.service&&<><div><dt>当前关联服务</dt><dd>{requestDetail.service.address||productNames[requestDetail.service.product]||requestDetail.service.product}</dd></div><div><dt>服务地区</dt><dd>{regionLabel(requestDetail.service.region)}{requestDetail.service.city?` / ${requestDetail.service.city}`:""}</dd></div><div><dt>关联订单</dt><dd className="mono">{requestDetail.service.orderId}</dd></div>{requestDetail.service.address&&<div><dt>当前 WiFi 名称</dt><dd>{requestDetail.service.wifiName||"未设置"}</dd></div>}</>}{requestDetail.type==="replace"&&requestDetail.previousAsset&&<><div><dt>更换前代理地址</dt><dd className="mono">{requestDetail.previousAsset.address}</dd></div><div><dt>更换前账号</dt><dd>{requestDetail.previousAsset.username||"未设置"}</dd></div><div><dt>更换前 WiFi 名称</dt><dd>{requestDetail.previousAsset.wifiName||"未设置"}</dd></div><div><dt>更换前协议</dt><dd>{requestDetail.previousAsset.protocol||"未设置"}</dd></div><div><dt>更换前地区</dt><dd>{requestDetail.previousAsset.country?regionLabel(requestDetail.previousAsset.country):"未设置"}{requestDetail.previousAsset.city?` / ${requestDetail.previousAsset.city}`:""}</dd></div></>}</dl><footer><button className="primary" onClick={()=>setRequestDetail(null)}>关闭详情</button></footer></section></div>}
    {checkout&&<div className="customer-payment-mask" onMouseDown={e=>{if(e.target===e.currentTarget)setCheckout(null)}}><section className="customer-payment-modal"><header><div><small>订单支付</small><h2>确认订单付款</h2></div><button onClick={()=>setCheckout(null)}>×</button></header><dl><div><dt>订单</dt><dd>{checkout.id}</dd></div><div><dt>商品</dt><dd>{checkout.bundleItems?`${checkout.bundleItems.length} 项商品 · ${checkout.quantity} 件`:`${productNames[checkout.product]||checkout.product} · ${regionLabel(checkout.region)} × ${checkout.quantity}`}</dd></div><div><dt>订单金额</dt><dd>${checkout.amount.toFixed(2)}</dd></div></dl><label>支付方式<select value={paymentMethod} onChange={event=>setPaymentMethod(event.target.value as "wallet"|"alipay")} disabled={externalPaymentOrderId===checkout.id}><option value="wallet">账户余额</option><option value="alipay">支付宝</option></select></label><label>优惠码（选填）<div className="coupon-apply"><input value={couponCode} disabled={externalPaymentOrderId===checkout.id} onChange={e=>{setCouponCode(e.target.value.toUpperCase());setDiscount(0);setCouponMessage("")}} placeholder="输入优惠码"/><button type="button" disabled={externalPaymentOrderId===checkout.id||validating||!couponCode.trim()} onClick={validateCoupon}>{validating?"验证中…":"使用"}</button></div></label>{couponMessage&&<p className={discount>0?"coupon-ok":"coupon-error"}>{couponMessage}</p>}<div className="payment-total"><span>应付金额{discount>0&&<small>已优惠 ${discount.toFixed(2)}</small>}</span><b>${Math.max(0,checkout.amount-discount).toFixed(2)}</b></div>{externalPaymentOrderId===checkout.id&&<p className="coupon-ok">请在支付宝页面完成付款，返回后点击下方按钮刷新订单状态。</p>}<footer><button onClick={()=>setCheckout(null)}>取消</button><button className="primary" disabled={paying===checkout.id} onClick={()=>externalPaymentOrderId===checkout.id?location.reload():paymentMethod==="alipay"?payAlipay(checkout):pay(checkout)}>{externalPaymentOrderId===checkout.id?"已完成支付":paying===checkout.id?"正在创建支付…":paymentMethod==="alipay"?"前往支付宝支付":"确认余额支付"}</button></footer></section></div>}
    ...
    {section==="requests"&&<section className="customer-service-request-history">
      ...
      <div className="service-request-history-table">
        ...
        {serviceRequests.length===0?<div className="empty">暂无售后申请记录</div>:serviceRequests.map(request=><div className="service-request-history-row" key={request.id}>
          <span><button type="button" className="customer-order-number mono" onClick={()=>setRequestDetail(request)}>{request.id}</button><small>{request.reason||"未填写原因"}</small></span>
